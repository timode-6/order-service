...
import com.example.order_service.service.order.OrderSecurityService;
...
class OrderControllerTest {
    ...
    void setUp() {
       UserResponse user = UserResponse.builder()
          ...
          .birthDate(LocalDate.of(2024, 12, 3)) 
          .active(true)
          .build();
        ...
        static class TestConfig implements WebMvcConfigurer {
                @Override
                public void addArgumentResolvers(@SuppressWarnings("rawtypes") List resolvers) {
                        resolvers.add(new org.springframework.security.web.method.annotation.AuthenticationPrincipalArgumentResolver());
                }
        }

        @BeforeEach
        void bypassFilter() throws ServletException, IOException {
        doAnswer(invocation -> {
                ...
                var auth = new UsernamePasswordAuthenticationToken("5", null, authorities);
        ...
        void created() throws Exception {
                doReturn(sampleResponse).when(orderService).createOrder(any());
        ...
        @Test
        @WithMockUser(username = "5")
        void foundAsOwner() throws Exception {
                when(orderSecurityService.isOwner(eq(10L), anyString())).thenReturn(true);
        ...
        void multipleStatuses() throws Exception {
            ...
            mvc.perform(get("/api/orders")
                            .param("statuses", "PENDING", "SUCCESS"))
                    .andExpect(status().isOk())
        ...
        void updated() throws Exception {
            OrderResponse updated = OrderResponse.builder()
                    .id(10L).status(OrderStatus.SUCCESS)
                    .totalPrice(2999L)
        ...
        @Test
        @DisplayName("404 when order not found")
        void deletedOrderNotFound() throws Exception {
